...
In this code chunk, there is only one line of code `1 + 2` which will output the result `3`. If necessary, you can include any lines of codes inside each code chunk. Each chunk can be viewed as a unit that usually serves a particular purpose. Inside each code chunk, you can run the codes **line by line** by the keyboard shortcut Cmd + Enter / Ctrl + Enter. However, it is often more convenient to run the codes **all at once** using the keyboard shortcut Shift + Cmd + Enter / Shift + Ctrl + Enter, or press the Green arrow on the right of the code chunk. 
...
- `{r pressure, echo=FALSE}` gives the code chunk the name "pressure", which is useful to navigate between different chunks, and it is also necessary to give the code chunk a name if you want to cross reference the corresponding figure in the knitted document.
...
In this example, we set `echo = TRUE` and `warning = FALSE`. You can feel free to change their values and add other kinds of customization. In each subsequent chunk, this set of global chunk options will be inherited. Of course, you can add additional options in each chunk to add additional chunk options or overwrite the global chunk options.

### Exercises

1. Create a new R Markdown document with the title "My First Report" and your name as the author. Knit it to HTML and verify the output.

2. In your R Markdown document, add a code chunk that assigns variables `x <- 10` and `y <- 20` and computes their sum. Set the chunk option `echo = FALSE` so that only the results appear in the output.

3. Add a second code chunk that displays a scatterplot using the built-in `plot(pressure)` dataset. Set the chunk option `fig.width = 8`.

4. Set global chunk options at the beginning of your document to have `echo = TRUE`, `warning = FALSE`, and `message = FALSE`. Verify that these options are inherited by subsequent chunks.
