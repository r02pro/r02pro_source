...
We can also construct date using the number of days since a reference date using the `origin` argument in the `as.Date()` function.
```{r, results=TRUE}
...
To create a time object from a character, you can use the `as.POSIXct()` function with the optional `format` argument.
```{r, results=TRUE}
