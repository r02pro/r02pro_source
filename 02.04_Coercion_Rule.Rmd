...
When we supply arguments of different types in the `c()` function, R will *unify* all elements into the **most general type**, which is usually called the **coercion rule**. "More general" means: a type that can represent every value of the simpler types. For example, every logical value (`TRUE` or `FALSE`) can be written as a number (`1` or `0`), and every number can be written as a string (`"1"`, `"0"`). The other direction is not possible — not every string is a number, and not every number is a logical value — so R always coerces toward the more general type.

The order of generality, from most specific to most general, is:
$$\mbox{logical} \;\rightarrow\; \mbox{numeric} \;\rightarrow\; \mbox{character}$$
