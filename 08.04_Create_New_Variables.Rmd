...
Note that you are free to use any functions on a vector, including all the arithmetic operations and various functions. For example,  to include the rank of each country in terms of GDP per capita in the year 2008, you can use `mutate(GDP_rank = rank(GDP_per_capita))`.  To create a variable as the mean of the `GDP_per_capita`  of all countries, you can add `GDP_per_capita_ave = mean(GDP_per_capita, na.rm = TRUE)` as an argument in the `mutate()` function.
...
gm %>%
  filter(year == 2008) %>%
  select(country, year, population, GDP_per_capita) %>%
  mutate(GDP_rank = rank(GDP_per_capita),
...
gm %>%
  filter(year == 2008) %>%
  select(country, year, population, GDP_per_capita) %>%
  transmute(GDP_rank = rank(GDP_per_capita),
            GDP_per_capita_ave = mean(GDP_per_capita, na.rm = TRUE))
```
