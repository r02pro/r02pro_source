...
Next, let's assign the value of `"book"` to a name (say `book_vec`) using the assignment operator `<-`. This is how you create a character vector with a name. 
...
As shown in the example, we can see that there are three elements in the `animals` vector, so `length(animals)` is 3. The string "sheep%29" has 8 characters (5 letters, 1 symbol, and 2 digits). Similarly, "bear$11" and "monkey@66" have 7 and 9 characters, and you can check it by yourself with the `nchar()` function. 
...
### A Deeper Look at R Help Documentation
