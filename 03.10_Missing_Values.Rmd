...
1. Think about what the value of `1^NA` is in R. Try to run it in R. Does it agree with your thoughts?

2. Think about what the value of `0^NA` is in R. Try to run it in R. Does it agree with your thoughts?
...
- *Answer 2.* We have $0^0 = \lim_{x\to 0}x^x =\exp[\lim_{x\to 0} x\log(x)]=\exp[0] = 1$, and $0^1 = 0$. Since `NA` represents uncertain values, the exponent could be 0, 1, or something else, so the result is not deterministic. Hence, the value of `0^NA` is `NA`.
:::
