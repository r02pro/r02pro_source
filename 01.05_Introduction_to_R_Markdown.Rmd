...
2. In your R Markdown document, add a code chunk that computes the mean and standard deviation of `sale_price` from the `sahp` dataset. Set the chunk option `echo = FALSE` so that only the results appear in the output.

3. Add a second code chunk that creates a scatterplot of `liv_area` vs. `sale_price` using **ggplot2**. Set the chunk option `warning = FALSE` and `fig.width = 8`.
