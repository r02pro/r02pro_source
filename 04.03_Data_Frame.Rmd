...
Looking at the data frame `my_data_frame`, it has 6 columns, each of which represents one variable. The variables are of different types. The `animal` is character, `year` is integer, both `weight` and `height` are doubles, `condition` is ordered factor (because we explicitly called `factor()`), and `healthy` is logical. Note that since R 4.0.0, `data.frame()` no longer automatically converts character vectors into factors.
