...
**Why `na.rm = TRUE` matters.** By default, most summary functions in R (including `mean()`, `median()`, `sum()`, `var()`, `sd()`) return `NA` if the input contains even a single missing value. Real-world data almost always has some `NA`s, so forgetting `na.rm = TRUE` is a common cause of surprising `NA` results in reports. If you only want to summarize the non-missing observations, remember to set `na.rm = TRUE`.

**American vs. British spelling.** The dplyr package accepts both `summarize()` (American) and `summarise()` (British) — they are identical. We use `summarize()` throughout this book for consistency.
:::
