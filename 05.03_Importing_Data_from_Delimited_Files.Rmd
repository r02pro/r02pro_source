...
We can see that there is a message showing the **Column specification** during the import process. In particular, we see `country`, `gender`, and `continent` are of type character, and `year` is of type double. We can also check the value of `gm_small` and its structure.
...
You will see that the imported data has only one variable named `country*year*gender*continent` since the function will interpret the whole thing as a single variable.
